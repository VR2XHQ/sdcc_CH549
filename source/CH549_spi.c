...
*******************************************************************************/
void SPIInterrupt(void) __interrupt(INT_NO_SPI0) __using(1) //SPI中断服务程序,使用寄存器组1
{
    UINT8 dat;
    dat = CH549SPISlvRead();
    CH549SPISlvWrite(dat^0xFF);
#if DE_PRINTF==0  //Debug message print via UART0
    printf("Read#%02x\n",(UINT16)dat);
#endif
}
#endif
